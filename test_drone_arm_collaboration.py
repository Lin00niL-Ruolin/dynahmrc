...
class DroneArmCollaborationTest:
    ...
    def setup_robots(self):
        ...
        try:
            ...
            print("\n   [创建] 固定机械臂 Bob...")
            # 机械臂底座位置 [0, 0.4, 1.3]
            # 桌子在 [0, 0, 0.8]，相对于机械臂是 [0, -0.4]，需要面向 -Y 方向
            import math
            yaw = -math.pi / 2  # -90度，面向 -Y 方向（桌子）
            orientation = [0, 0, math.sin(yaw/2), math.cos(yaw/2)]
            self.arm_robot = self.robot_factory.create_robot(
                robot_id="Bob",
                robot_type="arm",
                robot_model="panda",
                init_position=[0, 0.4, 1.3],  # 指定底座位置
                init_orientation=orientation  # 面向桌子
            )
            print(f"   [OK] 固定机械臂 Bob 创建成功")
            print(f"        基座位置: [0, 0.4, 1.3]")        
    ...
    def phase1_drone_pick_and_place(self):
        ...
        print("\n   [1.4] Lucy 导航到桌子上方...")
        # 桌子高度约0.8米，放置高度约0.9米
        place_pos = [table_pos[0], table_pos[1], table_pos[2]+0.3]
        hover_pos = [place_pos[0]-0.2, place_pos[1]-0.2, place_pos[2] + 3.0]
        hover_pos2 = [place_pos[0]-0.2, place_pos[1]-0.2, place_pos[2]+0.5]
        ...
        approach_pos = [-0.2, -0.2, place_pos[2]+0.36]  
